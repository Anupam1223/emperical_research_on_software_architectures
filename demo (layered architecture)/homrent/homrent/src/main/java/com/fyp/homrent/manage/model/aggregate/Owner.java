package com.fyp.homrent.manage.model.aggregate;
...
import com.fyp.homrent.manage.model.entities.Flat;

import org.springframework.data.domain.AbstractAggregateRoot;

@Entity
@NamedQueries({
        @NamedQuery(name = "Owner.findById", query = "select flat.bookingAmount from Owner where id = :flatId") })
public class Owner extends AbstractAggregateRoot<Owner> {
    ...
    public Owner(AddOwnerCommand addOwnerCommand) {
        ...
        flat = new Flat(addOwnerCommand.getLocation(), addOwnerCommand.getBooking_amount(),
                addOwnerCommand.getMonthly_rent(), addOwnerCommand.getType());
